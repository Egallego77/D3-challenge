...
xText.append("text") // append a "text" element to the xText group
  ...
  .text("In Poverty (%)"); // set the text to be a human readable label
...
yText.append("text") // append a "text" element to the yText group
  ...
  .text("Obesee (%)"); // set the text to be a human readable label
...
function visualize(data) {
  ...
  circlesGroup
    .append("circle") // append a 'circle' element to the circlesGroup
    ...
    .on("mouseover", function (d) { // .on 'mouseover' event, fire off a function that takes argument d
      ...
      d3.select(this).attr("stroke", "green"); // use d3.select() the this element, and modify the 'stroke' .style() to a color of your choosing
    })
    .on("mouseout", function (d) { // on 'mouseout' fire off a function that takes argument d
      ...
      // Remove highlight
      d3.select(this).attr("stroke", "red"); // use d3.select() the this element, and modify the 'stroke' .style() to another color of your choosing
    });
  ...
  circlesGroup.append("text") // append a 'text' element to circlesGroup
    ...
    .attr('dx', d => xScale(currentX)) // set the 'dx' attr() to map from d => xScale() applied to d[currentX]
    // When the size of the text is the radius,
    // adding a third of the radius to the height
    // pushes it into the middle of the circle.
    .attr("dy", d => yScale(currentY + circleRadius / 2.5)) // set the 'dy' attr() to map from d => yScale() applied to d[currentY]) + circleRadius / 2.5
  ...
  d3.selectAll(".axisText").on("click", function () {
    ...
    var selectedLabel = d3.select(this)//.attr("value"); // d3.select() the this (the thing that was clicked)
    ...
    if (selectedLabel.classed('inactive')) { // if the selectedLabel has the class 'inactive'
      ...
      if (axis === "x") { // if the axis is equal to 'x'
        ...
        // d3.selectAll() '.stateText' elements, and then use .each to fire off an anonymous function with no arguments
      ...
      else {  // Make currentX the same as the data name.
        currentY = name;
  ...
  function resizeChart() { // define a function called resizeChart that takes no arguments
    ...
    d3.select(".xAxis")
      .append("g") // select the .xAxis elements in the svg
      .call(xAxis) // call the xAxis function
      .attr("transform", "translate(0," + (height - margin - labelArea) + ")");

    d3.select(".yAxis")
      .append("g") // select the .yAxis elements in the svg
    ...
    d3.selectAll('circle') // d3.selectAll() 'circle' elements
      .attr("cy", d => yScale(currentY))// set the 'cy' attribute to use the yScale() d[currentY]
      .attr("cy", d => yScale(currentX)) // set the 'cy' attribute to use the xScale() d[currentX]
